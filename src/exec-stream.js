...
function ExecStream(command, args) {
  const execStream = this
  stream.Duplex.call(execStream)
  const proc = child_process.spawn(command, args)
  proc.stderr
    .setEncoding('utf-8')
    .on('data', function onStderrData(data) {
      console.log(data)
    })
  ...
  proc
    .on('exit', function onChildProcessExit(code) {
  ...
  proc.stdin
    .on('error', function onStdinError(err) {
      execStream.emit('error', err)
    })
  this.proc = proc
}
